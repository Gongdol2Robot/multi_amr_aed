...
- 깊이 안전 판단(`evaluate_depth_safety`)은 단일 최소값이 아니라 **ROI 내
  ...
  통합 운용 설정은 `allow_insufficient_depth_motion=false`이며,
  `INSUFFICIENT_DATA`도 장애물을 확인할 수 없는 상태로 보고 정지합니다.
  `compressedDepth` 구독자는 LiDAR FAULT로 fallback이 시작될 때만 만들고,
  복구·도착·실패 시 해제하므로 정상 Nav2 주행 중에는 Depth 영상이 추가로
  네트워크를 타지 않습니다.
...
- `SUCCEEDED → ALIVE`: 이미 fallback으로 목적지에 도착했으므로 완료된 Nav2
  ...
  오차만 기록한다.
- 목적지 도착 전 `→ ALIVE`: stable AMCL을 확인한 뒤 `RESUMED`를 발행한다.
  Mission Executor가 기존 event/version을 유지한 채 저장 goal로 Nav2를
  재개한다. 5초가 지나도 확인되지 않으면 계속 정지한다.
...
| `allow_insufficient_depth_motion` | `false` | depth 없음/유효 픽셀 부족 시 안전 정지 |
...
- 발행: `cmd_vel_nav`(Twist), `fallback_state`(String, `TRANSIENT_LOCAL`,
  `IDLE`/`STARTING`/`ACTIVE`/`BLOCKED`/`RECOVERING`/`RESUMED`/
  `SUCCEEDED`/`FAILED`), `recovery_ready`(Bool, stable AMCL 확인 완료),
...
- 액션 클라이언트:
  - `navigate_to_pose` — 취소는 `<action>/_action/cancel_goal` 서비스에 빈
    ...
    goal인지 몰라도 끌 수 있음). 복구 후 goal 재전송은 Mission Executor가
    담당한다. **경로 재계산은
...
fallback controller가 기본으로 실행된다.
