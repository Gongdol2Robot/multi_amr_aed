...
[CODE REVIEW]
robot1/robot2의 /scan을 독립적으로 감시하고 상태 머신 결과를 latched Topic으로 발행한다.
ROS I/O는 이 파일, 상태 전이 규칙은 lidar_state_machine.py가 담당한다.
"""
...
class LidarWatchdogNode(Node):
    ...
    # [CODE REVIEW]
    # 한 노드에서 여러 로봇을 감시하지만 RobotWatch/LidarMonitor는 로봇별로 따로 둔다.
    # 한 로봇의 scan 장애가 다른 로봇의 상태와 timeout 계산에 영향을 주지 않는다.
    ...
    def _on_scan(self, robot_name: str, _msg: LaserScan) -> None:
        # [CODE REVIEW] LaserScan 내용이 아니라 "새 메시지가 언제 도착했는가"만 본다.
        # 장애물 거리 품질 판단과 센서 통신 생존 판단의 책임을 분리하기 위해서다.
    ...
    def _handle_transition(
        ...
        # [CODE REVIEW] 상태가 실제로 바뀐 순간만 fault/recovery hook을 실행한다.
        # 주기 heartbeat 재발행은 hook을 다시 실행하지 않아 중복 취소/복구를 막는다.
    ...
    def _publish_state(self, robot_name: str, state: LidarState) -> None:
        # [CODE REVIEW] Bool은 단순 생존 확인용, String은 상세 상태 흐름 확인용이다.
        # TRANSIENT_LOCAL QoS라서 늦게 실행된 fallback 노드도 마지막 상태를 즉시 받는다.
        watch = self._robots[robot_name]
