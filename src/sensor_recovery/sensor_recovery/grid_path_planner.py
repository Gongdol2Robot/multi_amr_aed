...
[CODE REVIEW]
LiDAR 장애 뒤 Nav2 costmap/TF에 의존하지 않도록 latched static map만 사용하는 A*다.
최단거리보다 로봇 반경과 벽 여유 공간을 우선해 fallback 경로를 만든다.
"""
...
def compute_clearance_field(
    ...
    """
    # [CODE REVIEW] 각 cell에서 가장 가까운 벽까지의 실제 유클리드 거리를 미리 계산한다.
    # A*가 매 이웃 확장마다 모든 장애물을 다시 검색하지 않도록 만든 cost field다.
...
def plan_path(
    ...
    """
    # [CODE REVIEW] hard radius 안쪽 cell은 통과 금지하고, soft clearance 구간에는
    # 추가 cost를 부여해 조금 길어져도 벽에서 더 떨어진 경로를 선택한다.
...
def simplify_path(
    ...
    """
    # [CODE REVIEW] dense A* cell 경로를 그대로 따라가면 작은 조향이 반복된다.
    # line-of-sight마다 동일한 clearance 검사를 적용해 안전한 waypoint만 남긴다.
    if len(path) <= 2:
        return list(path)
