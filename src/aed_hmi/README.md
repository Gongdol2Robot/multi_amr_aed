...
각 타일의 **녹화** 버튼은 이미 수신한 MJPEG를 브라우저 Canvas에서 WebM으로
저장합니다. 별도 ROS 구독이나 로봇 영상 연결을 만들지 않으므로 로봇과 Wi-Fi
부하는 늘지 않으며, 중지하면 브라우저 다운로드 폴더에 바로 저장됩니다.
...
/{robot_id}/vision/debug/compressed         CompressedImage (preview 추론 결과)
