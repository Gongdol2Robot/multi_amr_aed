...
coordinator는 수동 관제 이벤트인 `/aed/emergency_event`뿐 아니라
`/camera_open/vision/emergency_event`와
`/camera_alley/vision/emergency_event`도 함께 구독합니다. 어느 카메라에서
출동이 시작됐든 동일한 event ID와 위치를 이어받아 현장 탐색을 시작합니다.
카메라가 이동 중 대상을 잠시 놓쳐 `CANCELED`를 발행해도 이미 확정된 이벤트
좌표는 폐기하지 않습니다. 중앙 출동과 동일하게 임무를 유지하고, 도착 뒤
조력자 호출과 복귀 절차를 계속 수행합니다.
