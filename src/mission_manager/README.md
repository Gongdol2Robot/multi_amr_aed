...
`mission_manager`는 직접 Nav2 경로를 계산하지 않습니다. 중앙의
`robot_state_monitor`가 이벤트별 실제 경로 길이를 계산해 `/aed/robot_state`로
보내며, 두 로봇의 `path_event_id` 응답을 기다린 뒤 선정합니다. 한쪽 응답이
없으면 기본 10초 제한시간 후 도착한 유효 후보만 사용합니다.
