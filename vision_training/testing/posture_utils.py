"""YOLO Pose의 COCO 관절과 bbox로 자세를 판정하는 공용 함수."""
...
class PostureHistory:
    ...
    def update(self, track_id: int, posture: str, frame_index: int) -> str:
        ...
        if history.count("FALLEN") >= self.fallen_hits:
            return "FALLEN"
        reliable = [
            value for value in history if value not in ("UNKNOWN", "FALLEN")
        ]
...
def classify_posture(
    ...
    shoulders = _center(keypoints, LEFT_SHOULDER, RIGHT_SHOULDER, keypoint_conf)
